...
def run_robustness():
    ...
    orig_returns = _run_single(oos, steps, daily_ret_panel, drop_etfs=None, seed=None)

    # Run N_RUNS with random drops
    all_run_returns = []
    for run in range(N_RUNS):
        print(f"  Run {run+1:2d}/{N_RUNS}...", end="", flush=True)
        run_returns = _run_single(oos, steps, daily_ret_panel, drop_etfs=N_DROP, seed=run)
...
def _run_single(oos, steps, daily_ret_panel, drop_etfs=None, seed=None):
    """Run a single backtest, optionally dropping N random ETFs at each step."""
