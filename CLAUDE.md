...
Logs: `logs/cron_backtest.log`, `logs/cron_pea.log`, `logs/cron_bourso_check.log`, `logs/trades.jsonl` (order history, one row per account per day, read by the webapp), `logs/accounts.json` (per-slot PEA cache: name, id, mail — regenerable, `python -m src.bourso.accounts`).
