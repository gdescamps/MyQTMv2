...
- `backtest.py` — `plot_backtest(price, alloc, nfci, cpi, last_days=None)`: 5-panel chart since 2000 (equity+SMA250, vol, allocation, NFCI, inflation); `last_days=252/21` re-uses the same layout windowed for the 1y/1m views. `swing_slow` marks the slow-bear red bands.
